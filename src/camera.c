...
fix16 prev = 0;
#define CAMERA_RIGHT_BORDER 200
#define CAMERA_LEFT_BORDER 50
void camera_update(struct camera* cam) {
    struct player_position p_pos = get_player_position();
    enum PLAYER_MOVE_DIRECTION p_dir = player_get_direction();

    if (fix16_to_int(p_pos.x) > CAMERA_RIGHT_BORDER && prev != p_pos.x &&  p_dir == DIRECTION_RIGHT) 
        move_camera_right(cam);
    
    if (fix16_to_int(p_pos.x) < CAMERA_LEFT_BORDER && prev != p_pos.x && p_dir == DIRECTION_LEFT)
