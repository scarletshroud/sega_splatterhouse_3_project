...
#define INJURED_STATE_OFFSET 5
static void update_state(struct zombie* const z, enum ZOMBIE_STATE state) {
    z->state = state + z->injured * INJURED_STATE_OFFSET;
}

#define IMPACT_DISTANCE 30
static void hit(struct zombie *z) {
    if (z->state != ZOMBIE_STATE_STAND && z->state != ZOMBIE_STATE_INJURED_STAND && z->state != ZOMBIE_STATE_INJURED_WALK && z->state != ZOMBIE_STATE_WALK && z->state != ZOMBIE_STATE_ATTACK) {
        return; 
    }
...
#define ANIM_DIE_TIME 29
static void handle_state(struct zombie* z) {
    switch (z->state) {
        case ZOMBIE_STATE_STAND:
        case ZOMBIE_STATE_WALK:
        case ZOMBIE_STATE_INJURED_STAND:
        case ZOMBIE_STATE_INJURED_WALK:

            if (walk(z)) {
                update_state(z, ZOMBIE_STATE_STAND);
                z->dx = FIX16(ZOMBIE_DEFAULT_DX);
                z->dy = FIX16(ZOMBIE_DEFAULT_DY);
            } else {
                update_state(z, ZOMBIE_STATE_WALK);
                update_position(z);
            }
...
static void attack(struct zombie* z) {
    if (frame_timer == 200 && (z->state == ZOMBIE_STATE_STAND || z->state == ZOMBIE_STATE_WALK || z->state == ZOMBIE_STATE_INJURED_STAND || z->state == ZOMBIE_STATE_INJURED_WALK)) {
